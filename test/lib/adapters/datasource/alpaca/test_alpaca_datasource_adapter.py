...
def test_run_query_delegates_to_fetch_historical_bars_for_historical_bars_type():
    adapter = AlpacaDatasourceAdapter(_make_config())
    query_config = _make_query_config(type="historical-bars")
    with patch.object(adapter, '_fetch_historical_bars', return_value=iter([])) as mock_method:
        adapter.run_query(query_config)
    called_with = mock_method.call_args[0][0]
    assert isinstance(called_with, HistoricalBarsQueryType)


def test_run_query_returns_results_from_fetch_historical_bars():
    adapter = AlpacaDatasourceAdapter(_make_config())
    query_config = _make_query_config(type="historical-bars")
    page = [MagicMock(), MagicMock()]
    with patch.object(adapter, '_fetch_historical_bars', return_value=iter([page])):
