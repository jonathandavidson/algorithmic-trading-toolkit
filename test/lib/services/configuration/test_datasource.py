...
def test_test_returns_name_on_success(tmp_path, monkeypatch):
    ...
    with patch("lib.services.datasources.alpaca.alpaca_service.config", MagicMock()):
...
def test_test_sends_correct_request(tmp_path, monkeypatch):
    ...
    with patch("lib.services.datasources.alpaca.alpaca_service.config", mock_sys_config):
        with patch("lib.services.datasources.alpaca.alpaca_service.requests.get", return_value=mock_response) as mock_post:
            datasource_service.test("alpaca-prod")
